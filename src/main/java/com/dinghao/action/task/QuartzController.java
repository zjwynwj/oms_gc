package com.dinghao.action.task;

import com.alibaba.fastjson.JSONObject;
import com.dinghao.process.task.service.QuartzService;
import com.dinghao.process.task.service.constant.JobEum;
import com.dinghao.process.task.service.constant.TaskUtil;
...
public class QuartzController {
    ...
    public String toAdd(HttpServletRequest request, HttpServletResponse response) throws SchedulerException {
        request.setAttribute("jobs", JobEum.getAll());
    ...
    public String add(HttpServletRequest request, HttpServletResponse response) throws SchedulerException, ClassNotFoundException {
        ...
        JobEum jobEum = JobEum.find(jobName);
        String cron = request.getParameter("cron");
        if (null == jobEum) {
            request.setAttribute("message", "没有任务!");
            request.setAttribute("opName", "添加失败!");
        } else {
            String groupName = TaskUtil.group_name;
            quartzService.addJob(jobEum.getJobName(), groupName, jobEum.getJobName(), groupName, jobEum.getCls(), cron);
        ...
        for (String triggerGroupName : triggerGroupNames) {
            ...
            for (TriggerKey triggerKey : triggerKeySet) {
                ...
                if (t instanceof CronTrigger) {
                    ...
                    JobEum jobEum = JobEum.find(jobKey.getName());
                    if (null == jobEum) {
                        jobInfo.setJobText("-");
                    } else {
                        jobInfo.setJobText(jobEum.getText());
                    }

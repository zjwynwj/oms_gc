...
$(function() {
 ...
 // 点击空白处，保存正在编辑的数据
    ...
    jQuery("#blackId").jqGrid({
        ...
        colNames: ['序号', 'gdsInfoId', 'receiptId', '操作', '商品编号','商品名称','规格代码','规格', '属性', '单位', '数量','单价','含税单价', '库位编码', '库位编码id', '备注'],
        colModel: [{name: 'id', index: 'id',  hidden: true },
                   {name: 'gdsInfoId', index: 'gdsInfoId', hidden: true},
                   {name: 'receiptId',  index: 'receiptId',    hidden: true     },
                   {name: 'act',index: 'act', hidden:  $("input[name='id']").val()>0?true:false, width: 80  },
                   {name: 'gdsInfoDetail',index: 'gdsInfoDetail', width: 250,
                     formatter: function(cellvalue, options, rowObject) {
                       return "<input value='" + cellvalue + "' width='250px' class='bindSelect' name='gdsInfoDetail' id=gdsShowInfo_" + options.rowId + ">";
                    }
                   ...
                   {name: 'gdsInfoCal',    index: 'gdsInfoCal', width: 40, hidden: true  },
                   {name: 'amount',   index: 'amount',editable: true, width: 80  },
                   {name: 'price',   index: 'price',editable: true, width: 80  },
                   {name: 'taxPrice',   index: 'taxPrice',editable: true, width: 80  },
                   {name: 'warehousePositionsName', index: 'warehousePositionsName',  width: 160,
            formatter: function(cellvalue, options, rowObject) {
...
function savePurOrder() {
   ...
   // $("#myForm").submit();
    ...
    for (var i in griddata) {
        ...
        receiptVo["receiptDetailVos[" + i + "].price"] = griddata[i].price;
        receiptVo["receiptDetailVos[" + i + "].taxPrice"] = griddata[i].taxPrice;

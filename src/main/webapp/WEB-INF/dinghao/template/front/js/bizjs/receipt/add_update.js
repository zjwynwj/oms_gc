...
$(function() {
    ...
    jQuery("#blackId").jqGrid({
        ...
        mtype: 'POST',
        colNames: ['序号', 'gdsInfoId', 'receiptId', '操作', '商品编号 名称 规格', '属性', '单位', '数量', '库位编码', '库位编码id', '备注'],
        colModel: [{
            name: 'id',
            index: 'id',
            hidden: true
        ...
        },
        {
            name: 'act',
            index: 'act',
            hidden:  $("input[name='id']").val()>0?true:false
        },
        {
            name: 'gdsInfoDetail',
            index: 'gdsInfoDetail',
            ...
            formatter: function(cellvalue, options, rowObject) {
                return "<input value='" + cellvalue + "' width='350px' class='bindSelect' name='gdsInfoDetail' id=gdsShowInfo_" + options.rowId + ">";
            }
        },
        {
            name: 'gdsInfoFormat',
        ...
        { name: 'gdsInfoCal',index: 'gdsInfoCal',width:30 },
        {
            name: 'amount',
            index: 'amount',
            editable: true,
        ...
        {
            name: 'warehousePositionsId',
            index: 'warehousePositionsId',
            hidden: true
        },
        {
            name: 'remarks',
    ...
    $('#warehouseName').select2({
        ...
        ajax: {
            url: base_template + "/warehouse/get_warehouses_select2.jhtml",
            delay: 250,
            dataType: 'json',
            data: function(term, page) {
                return {
                    warehouseName: encodeURI(decodeURIComponent(term, true)),
                    // 汉字处理方式
                    pageNum: 1,
                    // 页码
                    rows: 15,
...
$('#myForm').ajaxForm({
    ...
    success: function(data) {
        if (data.result) {
            var receiptId = data.t;
            var warehouseId =  $("#warehouseId").val();
            $("#receiptId").val(receiptId);
            var receiptDetailVos = {};
            var griddata = $("#blackId").getRowData();
            receiptDetailVos["id"] = receiptId;
            receiptDetailVos["warehouseId"] = warehouseId;
            receiptDetailVos["receiptType"] = 1;
            for (var i in griddata) {
                receiptDetailVos["receiptDetailVos[" + i + "].id"] = griddata[i].id;
                receiptDetailVos["receiptDetailVos[" + i + "].gdsInfoId"] = griddata[i].gdsInfoId;
                receiptDetailVos["receiptDetailVos[" + i + "].receiptId"] = receiptId;
                if (griddata[i].id == '') {
                    receiptDetailVos["receiptDetailVos[" + i + "].gdsInfoDetail"] = $(griddata[i].gdsInfoDetail).find(".select2-chosen").html();
                    ...
                    receiptDetailVos["receiptDetailVos[" + i + "].gdsInfoDetail"] = gdsInfoDetail;
                    // ------------------------------------------------------------------------
                    ...
                    receiptDetailVos["receiptDetailVos[" + i + "].warehousePositionsName"] = warehousePositionsName;
                }
                receiptDetailVos["receiptDetailVos[" + i + "].gdsInfoFormat"] = griddata[i].gdsInfoFormat;
                receiptDetailVos["receiptDetailVos[" + i + "].gdsInfoCal"] = griddata[i].gdsInfoCal;
            ...
            options.url = base_template + "/receipt/save_receipt_details.jhtml";

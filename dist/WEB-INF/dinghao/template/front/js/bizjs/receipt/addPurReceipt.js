...
$(function() {
    ...
    jQuery("#blackId").jqGrid({
        ...
        mtype: 'POST',
        colNames: ['序号', 'gdsId', '商品编号 名称 规格', '规格代码','属性', '单位', '订单数量','已入库数量', '入库数量','库位编码', '库位编码id', '备注'],
        colModel: [
            {name: 'id', index: 'id', hidden: true},
            {name: 'gdsId', index: 'gdsId', hidden: true  },
            { name: 'gdsShowInfo', index: 'gdsShowInfo', width: 350,
	        ...
	        {name: 'purAmount',index: 'purAmount', width: 60},
	        {name: 'purRealAmount',index: 'purRealAmount', width: 80},
	        ...
	        {name: 'warehousePositionsName',index: 'warehousePositionsName',width: 200,
	            formatter: function(cellvalue, options, rowObject) {
	                ...
	                return "<input value='" + cellvalue + "' width='200px' class='bindSelect' name='warehousePositionsCode' id=warehousePositionsCode_" + options.rowId + ">";
	            }
	        },
	    ...
	    $("#warehouseId").change(
	    		   function() {
	    			  wid = $(this).val();
	    			   //获取默认库位
	    			   var options = {};
	    			    options.params = {"id": wid};
	    			    // 没有订单编号 自动生成
	    			    options.url = base + "/template/warehouse/queryDefaultPosition.jhtml";
	    			    	 ...
	    			    	 var purData = $("#blackId").getRowData();
	    			    	    for (var i in purData) {
	    			    	        ...
	    			    	        if ($.trim(row.warehousePositionsId) == "" || $.trim(row.warehousePositionsId) == undefined) {
	    			    	        	   ...
	    			    	        	   jQuery("#blackId").setCell(rowId, "warehousePositionsId", data.data.id);
	    			    	               $("#warehousePositionsCode_" + rowId).val(data.data.warehousePositionsCode);
		    			    	           $("#warehousePositionsCode_" + rowId).prev("div").find(".select2-chosen").html(data.data.warehousePositionsCode)
...
function gridDataisPerfect() {
    ...
    for (var i in purData) {
    	...
    	if(parseFloat(oms.numSub(row.purAmount , row.purRealAmount)) < parseFloat(row.amount)){
    		oms.tooltip("入库数量不能超过当前的未入库数量!", "error");
            return false;
    	}
    	totalAmout += row.amount;
    }
    if(parseFloat(totalAmout) == 0){
		oms.tooltip("入库数量不能为0","error" , 5000);
		return false;
	}
    return true;
}
...
function bindWarehousePositionsName(e) {
    $(e).select2({
        ...
        formatSelection: function(item) { // 选中后显示在文本框中的值
            ...
            $("#warehousePositionsCode_" + rowId).val(item.warehousePositionsCode);
